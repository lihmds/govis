...
class BoardProbabilityDisplay:
  def __init__(self, board_size, viewport_size):
    self.board_size = board_size
    self.viewport_size = viewport_size
    self.tile_radius = viewport_size / (2 * board_size)
  ...
  def draw_background(self):
    margin = 10
    background_size = self.viewport_size + 2*margin
  ...
  def draw_stone(self, center, opacity, color):
    ...
    circle = pyglet.shapes.Circle(screen_x, screen_y, radius = self.tile_radius)
  ...
  def tile_center(self, x, y):
    interpolate_x = interp1d([0, self.board_size-1], [self.tile_radius, self.viewport_size-self.tile_radius])
    # y increases upwards in pyglet's coordinates, but downwards in board coordinates
    interpolate_y = interp1d([0, self.board_size-1], [self.viewport_size-self.tile_radius, self.tile_radius])
